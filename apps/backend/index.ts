...
app.post("/ai/training", authMiddleware, async (req, res) => {
  try {
    ...
    // );
    const { request_id } = await openaiModel.trainModel(
      parsedBody.data.zipUrl,
    );
...
app.post("/ai/generate", authMiddleware, async (req, res) => {
  ...
  //   model.tensorPath,
  // );
  const _request_id = await openaiModel.generateImage(
    parsedBody.data.prompt,
  );
...
app.post("/pack/generate", authMiddleware, async (req, res) => {
  ...
  //   ),
  // );
  let requestIds: string[] = await Promise.all(
    prompts.map((prompt) => openaiModel.generateImage(prompt.prompt))
  );
  ...
  const images = await prismaClient.outputImages.createManyAndReturn({
    data: prompts.map((prompt, index) => ({
      ...
      modelId: parsedBody.data.modelId,
      imageUrl: "",
      falAiRequestId: requestIds[index], // <-- now a string
    })),
  });
